...
The config file should be located at: `{{VAULT_ROOT}}/99_System/Config/research_interests.yaml`
...
```bash
# {{VAULT_ROOT}} 由 AGENTS.md 占位符映射解析为实际 Vault 路径
LANGUAGE=$(grep -E "^\s*language:" "{{VAULT_ROOT}}/99_System/Config/research_interests.yaml" | awk '{print $2}' | tr -d '"')
...
2. **读取研究配置**
   - 读取 `{{VAULT_ROOT}}/99_System/Config/research_interests.yaml`（注意：文件名是 interests 不是 interest）获取研究领域
   - 提取：关键词、类别和优先级
...
python scripts/search_arxiv.py \
  --config "{{VAULT_ROOT}}/99_System/Config/research_interests.yaml" \
