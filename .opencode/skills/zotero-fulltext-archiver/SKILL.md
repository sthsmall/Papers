...
>
> **询问只在首次确认缺失时发生一次**：一旦环境中 MinerU 已被确认可用（或用户已明确授权安装、已完成安装），后续所有调用**直接使用，不再重复询问**。每次任务开始时只需一次轻量探测（`Test-Path` / 命令探测）；探测成功即直接进入后续步骤。
>
> **不适用场景**：若用户已授权安装、或已有可用 MinerU（含历史输出可直接复用），则无需询问，直接按第 2、3 节处理。
